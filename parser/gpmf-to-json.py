...
# Set up logging
logging.basicConfig(level=logging.DEBUG, filename='gpmf2json.log', filemode='w')
...
def get_valid_input(argv):
    ...
    if len(argv) < 3:
        logger.error('Two inputs are required.')
        print(
    ...
    if input_path == output_path:
        logger.error('Input and output cannot be the same.')
        print(
            'ERROR: input and output cannot be the same.\nUse the format "python gpmf2json.py [input mp4/mov file] [output json file]" for a single file\nor  "python gpmf2json.py [input directory] [output directory]" for batch processing.'
        )
        exit()
    if not (os.path.isdir(input_path) == os.path.isdir(output_path)):
        logger.error('Please specify either two files or two directories as an input.')
        print(
    ...
    for gpmf_data, timestamps in payloads:
        ...
        for element, parents in recursive(gpmf_data):
            try:
                ...
                value = element.data
            key_path = [x.decode("latin-1") for x in list(parents) + [element.key] if x.decode("latin-1")]
            # Ensure no empty keys
            if "" in key_path:
                logger.debug(f"Empty key detected in key_path: {key_path}")
                key_path = ["UNKNOWN_KEY" if not k else k for k in key_path]
            data_entry.append(
                (key_path, value)
            )
    ...
    if os.path.isdir(input_path):
        ...
        return list(
            map(
                ...
                filter(
                    lambda x: os.path.splitext(x)[-1].lower() in [".mp4", ".mov"],
                    os.listdir(input_path),
                ),
            )
        )
    else:
        # converts single input/output files to [(input_path, output_path)]
        return [(input_path, output_path)]
...
if __name__ == "__main__":
    ...
    for infile, outfile in files:
        try:
            ...
            logger.error(f"Error processing file '{infile}': {e}")
